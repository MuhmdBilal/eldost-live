...
function Bronze() {
  ...
  const handleMint = async () => {
    try {
      if (walletAddress) {
        ...
        const collection  = await wmpNftContract.methods.collections(0).call()
        const balanceOf = await usdtContract.methods.balanceOf(walletAddress).call()
        const calculateValue = Number(collection?.price)/1000000;
        if(Number(balanceOf) >= calculateValue){
          await usdtContract.methods.approve(WMPNFTAddress, Number(collection?.price)).send({
            from: walletAddress,
          });
          await wmpNftContract.methods.mint(0).send({
            from: walletAddress,
          });
    ...
    try {
      if (walletAddress) {
        ...
        const response = walletOfOwner
          .filter((value) => Number(value) >= 1 && Number(value) <= 125)
          .map((value) => ({
            value: Number(value),
  ...
  return (
    <div className="bronze" style={{ padding: "20% 0 0 0" }}>
      <div className="container py-4 ">
        ...
        <div className="row">
          <div className="col-2 mt-2">
            <button
              className=" btn btn-success rounded-1 w-100 fs-4"
              onClick={handleMint}
              disabled={isLoading}
            >
        ...
        <div className=" col-md-12 border rounded-3 mt-3 p-3 d-grid pt-5">
          ...
          <div className="col-3 mt-3">
            <button className="btn btn-success" onClick={handleStaking}
