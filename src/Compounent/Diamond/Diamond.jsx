...
function Diamond() {
  ...
  const handleMint = async () => {
    try {
      if (walletAddress) {
        ...
        const collection = await wmpNftContract.methods.collections(4).call();
        ...
        if (Number(balanceOf) >= calculateValue) {
          ...
          await wmpNftContract.methods.mint(4).send({
            from: walletAddress,
          });
    ...
    try {
      if (walletAddress) {
        ...
        const response = walletOfOwner
          .filter((value) => Number(value) >= 351 && Number(value) <= 375)
          .map((value) => ({
            value: Number(value),
  ...
  return (
    <div className="diamond" style={{ padding: "20% 0 0 0" }}>
