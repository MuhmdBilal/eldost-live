...
function Platinum() {
  ...
  const handleMint = async () => {
    try {
      if (walletAddress) {
        ...
        const collection  = await wmpNftContract.methods.collections(3).call()
        ...
        if(Number(balanceOf) >= calculateValue){
          ...
          await wmpNftContract.methods.mint(3).send({
            from: walletAddress,
          });
    ...
    try {
      if (walletAddress) {
        ...
        const response = walletOfOwner
          .filter((value) => Number(value) >= 301 && Number(value) <= 350)
          .map((value) => ({
            value: Number(value),
  ...
  return (
    <div className="platinum" style={{ padding: "20% 0 0 0" }}>
